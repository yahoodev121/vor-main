...
class ProgramControlTemplateRepository {
  ...
  static async create(data, options: IRepositoryOptions) {
    ...
    const [record] = await ProgramControlTemplate(
      options.database,
    ).create(
      [
        {
  ...
  static async update(
    ...
    let record =
      await MongooseRepository.wrapWithSessionIfExists(
        ProgramControlTemplate(options.database).findOne({
          _id: id,
          tenant: currentTenant.id,
        }),
        options,
  ...
  static async filterIdsInTenant(
    ...
    const records = await ProgramControlTemplate(
      options.database,
    )
      .find({
        _id: { $in: ids },
  ...
  static async findById(
    ...
    let record =
      await MongooseRepository.wrapWithSessionIfExists(
        ProgramControlTemplate(options.database)
          .findOne({ _id: id })
          .populate({
  ...
  static async findAndCountAll(
    ...
    if (filter) {
      ...
      if (
        filter.byRequirementTemplates === true ||
        filter.byRequirementTemplates === 'true'
      ) {
        MongooseQueryUtils.pushInCriteria(
          criteriaAnd,
    ...
    let rows = await ProgramControlTemplate(
      options.database,
    )
      .find(criteria)
      .skip(skip)
      ...
      .populate({
        path: 'requirementTemplates',
        select: ['_id', 'name', 'tenant', 'requirementID'],
      });

    const count = await ProgramControlTemplate(
      options.database,
  ...
  static async _createAuditLog(
    ...
    await AuditLogRepository.log(
      {
        entityName: ProgramControlTemplate(options.database)
          .modelName,
        entityId: id,
        action,
        values: data,
  ...
  static async _mapRelationshipsAndFillDownloadUrl(
    ...
    output.requirementTemplateIDs =
      output.requirementTemplates.map(
        (item) => item.requirementID,
      );

    output.requirementTemplateNames =
      output.requirementTemplates.map((item) => item.name);
...
export default ProgramControlTemplateRepository;
