...
class RiskCategoryRepository {
  static async create(data, options: IRepositoryOptions) {
    ...
    const [record] = await RiskCategory(
      options.database,
    ).create(
      [
        {
  ...
  static async update(
    ...
    let record =
      await MongooseRepository.wrapWithSessionIfExists(
        RiskCategory(options.database).findOne({
          _id: id,
          tenant: currentTenant.id,
        }),
        options,
    ...
    await RiskCategory(options.database).updateOne(
      { _id: id },
      {
        ...data,
        updatedBy:
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await RiskCategory(options.database).deleteOne(
      { _id: id },
      options,
    );
  ...
  static async replaceIdsInTenant(
    ...
    for (const cat of cats) {
      ...
      const category = await RiskCategory(
        options.database,
      ).findOneAndUpdate(
        {
          name: cat,
  ...
  static async filterIdsInTenant(
    ...
    const records = await RiskCategory(options.database)
      .find({
        _id: { $in: ids },
        tenant: currentTenant.id,
      })
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return MongooseRepository.wrapWithSessionIfExists(
      RiskCategory(options.database).countDocuments({
        ...filter,
        tenant: currentTenant.id,
      }),
      options,
  ...
  static async findAndCountAll(
    ...
    let rows = await RiskCategory(options.database)
      .find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort);

    const count = await RiskCategory(
      options.database,
  ...
  static async _createAuditLog(
    ...
    await AuditLogRepository.log(
      {
        entityName: RiskCategory(options.database)
          .modelName,
        entityId: id,
        action,
        values: data,
...
export default RiskCategoryRepository;
