...
class ClientRepository {
  static async create(data, options: IRepositoryOptions) {
    ...
    const [record] = await Client(options.database).create(
      [
        {
          ...data,
    ...
    await FileRepository.assignRelatedData(
      ...
      {
        type: FileRepository.TYPE_CLIENT,
        typeId: record.id,
  ...
  static async addUserAsSameMember(
    ...
    await Client(options.database).updateMany(
      {
  ...
  static async update(
    ...
    let record =
      await MongooseRepository.wrapWithSessionIfExists(
        Client(options.database).findOne({
          _id: id,
          tenant: currentTenant.id,
        }),
        options,
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await Client(options.database).deleteOne(
      { _id: id },
      options,
    );
    ...
    await FileRepository.releaseRelatedData(
      { type: FileRepository.TYPE_CLIENT, typeId: id },
      options,
    );
  ...
  static async filterToCriteria(
    ...
    if (filter) {
      ...
      MongooseQueryUtils.pushEqualCriteria(
        criteriaAnd,
        filter,
        ['status', 'rating', 'industry', 'gdprRopa'],
      );
  ...
  static async findAllIds(
    ...
    const rows = await Client(options.database)
      .find(criteria)
      .select(['_id']);
  ...
  static async findAndCountAll(
    ...
    if (!filter?.export && metaOnly) {
      if (withOnlyUsers) {
        rows = await Client(options.database)
          .find(criteria)
          .skip(skip)
          .limit(limitEscaped)
          .sort(sort)
  ...
  static async findByUser(
    ...
    let rows = await Client(options.database).find({
      users: userId,
      tenant: currentTenant.id,
    });
