...
class TaskListRepository {
  ...
  static async update(
    ...
    let record =
      await MongooseRepository.wrapWithSessionIfExists(
        TaskList(options.database).findOne({
          _id: id,
          tenant: currentTenant.id,
        }),
        options,
    ...
    await TaskList(options.database).updateOne(
      { _id: id },
      {
        ...data,
        updatedBy:
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await TaskList(options.database).deleteOne(
      { _id: id },
      options,
    );
  ...
  static async replaceIdInTenant(
    ...
    return lodash.get(
      await this.replaceIdsInTenant([taskList], options),
      '[0]',
      null,
    );
  }
  ...
  static async replaceIdsInTenant(
    ...
    for (const name of taskLists) {
      if (MongooseQueryUtils.isValidObjectId(name)) {
        ids.push(name);
        continue;
      }
  ...
  static async filterIdsInTenant(
    ...
    const records = await TaskList(options.database)
      .find({
        _id: { $in: ids },
        tenant: currentTenant.id,
      })
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return MongooseRepository.wrapWithSessionIfExists(
      TaskList(options.database).countDocuments({
        ...filter,
        tenant: currentTenant.id,
      }),
      options,
  ...
  static async findAndCountAll(
    ...
    let rows = await TaskList(options.database)
      .find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort);

    const count = await TaskList(
      options.database,
  ...
  static async findAllAutocomplete(
    ...
    const records = await TaskList(options.database)
      ...
      .limit(limitEscaped)
      .sort(sort)
      .select(['_id', 'name', 'taskdisplaycolor']);

    return records.map((record) => ({
      id: record.id,
      label: record.name,
      color: record.taskdisplaycolor,
    }));
  }
  ...
  static async _createAuditLog(
    ...
    await AuditLogRepository.log(
      {
        entityName: TaskList(options.database).modelName,
        entityId: id,
        action,
        values: data,
      },
