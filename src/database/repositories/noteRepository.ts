...
class NoteRepository {
  static async create(data, options: IRepositoryOptions) {
    ...
    const [record] = await Note(options.database).create(
      [
        {
          ...data,
  ...
  static async update(
    ...
    await Note(options.database).updateOne(
      { _id: id },
      {
        ...data,
        updatedBy:
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    let notes =
      await MongooseRepository.wrapWithSessionIfExists(
        Note(options.database).countDocuments({
          notes: id,
          tenant: currentTenant.id,
        }),
        options,
    ...
    await Note(options.database).deleteOne(
      { _id: id },
      options,
    );
  ...
  static async filterIdsInTenant(
    ...
    const records = await Note(options.database)
      .find({
        _id: { $in: ids },
        tenant: currentTenant.id,
      })
  ...
  static async findAndCountAll(
    ...
    let rows = await Note(options.database)
      .find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort)
  ...
  static async _createAuditLog(
    ...
    await AuditLogRepository.log(
      {
        entityName: Note(options.database).modelName,
        entityId: id,
        action,
        values: data,
      },
