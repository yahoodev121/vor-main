...
import ProjectStatusRepositoryEx from './extend/projectStatusRepositoryEx';
...
class ProjectStatusRepository {
  static async create(data, options: IRepositoryOptions) {
    ...
    const [record] = await ProjectStatus(
      options.database,
    ).create(
      [
        {
          ...
          order:
            data.order ??
            ProjectStatusRepositoryEx.DEFAULT_STATUS_NAMES
              .length,
  ...
  static async update(
    ...
    await ProjectStatus(options.database).updateOne(
      { _id: id },
      {
        ...data,
        updatedBy:
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await ProjectStatus(options.database).deleteOne(
      { _id: id },
      options,
    );
  ...
  static async replaceIdInTenant(
    ...
    return lodash.get(
      await this.replaceIdsInTenant([taskStatus], options),
      '[0]',
      null,
    );
  }
  ...
  static async replaceIdsInTenant(
    ...
    for (const status of projectStatuses) {
      if (MongooseQueryUtils.isValidObjectId(status)) {
        ids.push(status);
        continue;
      }
  ...
  static async filterIdsInTenant(
    ...
    const records = await ProjectStatus(options.database)
      .find({
        _id: { $in: ids },
        tenant: currentTenant.id,
      })
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return MongooseRepository.wrapWithSessionIfExists(
      ProjectStatus(options.database).countDocuments({
        ...filter,
        tenant: currentTenant.id,
      }),
      options,
  ...
  static async findById(id, options: IRepositoryOptions) {
    ...
    let record =
      await MongooseRepository.wrapWithSessionIfExists(
        ProjectStatus(options.database)
          .findOne({
            _id: id,
            tenant: currentTenant.id,
          })
  ...
  static async _createAuditLog(
    ...
    await AuditLogRepository.log(
      {
        entityName: ProjectStatus(options.database)
          .modelName,
        entityId: id,
        action,
        values: data,
