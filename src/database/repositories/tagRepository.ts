...
class TagRepository {
  ...
  static async filterIdsInTenant(
    ...
    for (const id of ids) {
      ...
      if (MongooseQueryUtils.isValidObjectId(tag)) {
        if (!filteredIds.includes(tag)) {
          filteredIds.push(tag);
        }
        continue;
      }
      ...
      const dbId = record.id.toLowerCase();
      if (!filteredIds.includes(dbId)) {
        filteredIds.push(dbId);
      }
    }
    ...
    return records.map((record) => record.id.toLowerCase());
  }
