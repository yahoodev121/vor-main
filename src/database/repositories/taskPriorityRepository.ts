...
class TaskPriorityRepository {
  static async create(data, options: IRepositoryOptions) {
    ...
    const [record] = await TaskPriority(
      options.database,
    ).create(
      [
        {
          ...
          order:
            data.order ??
            TaskPriorityRepositoryEx.DEFAULT_PRIORITY_NAMES
              .length,
  ...
  static async update(
    ...
    await TaskPriority(options.database).updateOne(
      { _id: id },
      {
        ...data,
        updatedBy:
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await TaskPriority(options.database).deleteOne(
      { _id: id },
      options,
    );
  ...
  static async replaceIdsInTenant(
    ...
    if (!taskPriorities || !taskPriorities.length) {
      return [];
    }
    ...
    for (const priority of taskPriorities) {
      ...
      const taskPriority = await TaskPriority(
        options.database,
      ).findOneAndUpdate(
        {
          priority: priority,
  ...
  static async filterIdsInTenant(
    ...
    const records = await TaskPriority(options.database)
      .find({
        _id: { $in: ids },
        tenant: currentTenant.id,
      })
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return MongooseRepository.wrapWithSessionIfExists(
      TaskPriority(options.database).countDocuments({
        ...filter,
        tenant: currentTenant.id,
      }),
      options,
  ...
  static async _createAuditLog(
    ...
    await AuditLogRepository.log(
      {
        entityName: TaskPriority(options.database)
          .modelName,
        entityId: id,
        action,
        values: data,
