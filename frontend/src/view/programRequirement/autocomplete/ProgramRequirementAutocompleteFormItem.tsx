...
function ProgramRequirementAutocompleteFormItem(props) {
  ...
  const fetchFn = (value, limit) => {
    return ProgramRequirementService.listAutocomplete(
      value,
      limit,
    );
  };
  ...
  const mapper = {
    toAutocomplete(originalValue) {
      ...
      if (originalValue.name) {
        label = `${originalValue.requirementID} ${originalValue.name}`;
      }
...
export default ProgramRequirementAutocompleteFormItem;
