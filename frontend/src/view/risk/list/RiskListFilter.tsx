...
const schema = yup.object().shape({
  ...
  summary: yupFilterSchemas.string(
    i18n('entities.risk.fields.summary'),
  ),
});
...
const previewRenders = {
  ...
  summary: {
    label: i18n('entities.risk.fields.summary'),
    render: filterRenders.enumerator(
      'entities.risk.enumerators.summary',
    ),
  },
};
...
function RiskListFilter(props) {
  ...
  useEffect(() => {
    if (['open'].includes(initialValues.summary)) {
      form.register('summary');
      form.setValue('summary', initialValues.summary, {
