...
function TaskFormModal(props) {
  ...
  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      if (mode === 'edit') {
        await TaskService.update(record.task?.id, data);
        ...
        {dispatched && !initLoading && (
          <TaskForm
            ...
            record={{
              ...record,
              ...resetInfos,
              highlights: record?.task?.highlights || [],
              newsArticles:
                record?.task?.newsArticles || [],
              policies: record?.task?.policies || [],
              policyTemplates:
                record?.task?.policyTemplates || [],
              products: record?.task?.products || [],
            }}
