...
const policyFormActions = {
  ...
  doInit: (id, version) => async (dispatch) => {
    try {
      dispatch({
      ...
      if (isEdit) {
        record = await PolicyService.find(id, version);
      }
  ...
  doCreate: (values) => async (dispatch) => {
    try {
      ...
      const record = await PolicyService.create(values);
      ...
      getHistory().push(`/policy/${record.id}`);
    } catch (error) {
      Errors.handle(error);
  ...
  doUpdate: (id, values) => async (dispatch, getState) => {
    try {
      ...
      const record = await PolicyService.update(id, values);
      ...
      getHistory().push(`/policy/${id}/${record.version}`);
    } catch (error) {
      Errors.handle(error);
